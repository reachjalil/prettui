import { clamp, dim, joinAligned, rgb, selectedLine, theme, truncate } from "../ansi";
import type { FooterItem } from "../components";
import { renderBrandHeader, renderFooter, renderPanel } from "../components";
import { divider, hstack, normalizeFrame, renderKeyValueRow, splitRatioWidths } from "../layout";
...
export function reduceTuiAppEvent<TContext>(definition: TuiAppDefinition<TContext>, state: TuiAppState, event: TuiAppEvent): TuiAppState {
  ...
  if (event.type === "menu") {
    ...
    if (!menu) throw new Error(`Unknown preTUIy menu id: ${event.menuId}`);
    return {
      ...state,
