...
function renderChoice4(options: TuiDemoRenderOptions): string {
  ...
  return renderPermissionWorkflowLayout({
    ...
    height: options.height,
    color: options.color,
    brand: { title: "preTUIy", subtitle: "automation realtime analytics TUI" },
    header: [
      {
        title: "Session",
        status: "LIVE",
    ...
    footer: { status: [{ label: "preTUIy", value: "v0.1.0", tone: "white" }], controls: controlFooter(paused, controls, rows.length) },
    selectedIndex: clampIndex(controls.selectedEventIndex, rows.length),
  });
}
  ...
  if (component.id === "brand-header") {
    return renderPanel({
      ...
      children: renderBrandHeader({
        title: variant === "library" ? "preTUIy" : variant === "minimal" ? "TUI" : "preTUIy",
